...
type AppLogoProps = AnchorHTMLAttributes<HTMLAnchorElement> & {
    href?: string;
};

export const AppLogo = forwardRef<HTMLAnchorElement, AppLogoProps>(({ href, className, ...props }, ref) => {
    const compProps = {
        className: cn(
    ...
    if (href) {
        return (
            <Link href={href} {...compProps} ref={ref}>
                {logoContent}
            </Link>
        );
    }

    return <span {...compProps}>{logoContent}</span>;
});
