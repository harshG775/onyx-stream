    ...
    return (
        <>
            <header className="sticky top-0 z-50  border-b">
                <nav className="max-w-6xl mx-auto h-16 w-full flex items-center gap-2 p-2">
                    <button className=" cursor-pointer h-9 px-4 py-2 flex items-center rounded-md font-bold text-2xl">
                        <LOGO className="size-8" />
                        <div className="text-primary">Onyx</div>Stream
                    </button>
                    {StreamMode.map((item) => (
                        <Button key={item.id} variant={"link"} className="cursor-pointer">
                            {item.name}
                        </Button>
                    ))}
                </nav>
...
const LOGO = ({ ...props }: React.ComponentProps<"svg">) => (
    <svg xmlns="http://www.w3.org/2000/svg" width="640" height="512" viewBox="0 0 640 512" {...props}>
        <path
            fill="currentColor"
